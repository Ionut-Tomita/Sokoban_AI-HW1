...
class Map:
    ...
    def is_valid_move(self, move):
        ''' Checks if the move is valid (only push moves allowed) '''
        if move < BOX_LEFT:
            return self.player_valid_move(move)
        elif move <= BOX_DOWN:
            # Verificăm dacă mișcarea este un push
            future_position = self.player.get_future_position(move - 4)
            if future_position in self.positions_of_boxes:
    ...
    def serialize_state(self):
        # Serializăm poziția jucătorului și pozițiile cutiilor
        player_position = (self.player.x, self.player.y)
        box_positions = tuple(sorted((box.x, box.y) for box in self.boxes.values()))
        return (player_position, box_positions)
